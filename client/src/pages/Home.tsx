...
export default function Home() {
  ...
  const createAgreement = () => {
    ...
    setCreateOpen(false);
    toast.success("Agreement staged", { description: "Prototype state saved. No funds moved." });
  };
    ...
    <main className="min-h-screen overflow-hidden bg-ink text-bone">
      <header className="sticky top-0 z-30 border-b border-white/10 bg-ink/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-[1440px] items-center justify-between px-5 py-4 lg:px-8">
          ...
          <div className="flex items-center gap-3">
            <span className="hidden rounded-full border border-chartreuse/30 bg-chartreuse/10 px-3 py-1.5 font-mono text-[9px] uppercase tracking-[0.15em] text-chartreuse sm:inline-flex">Prototype mode</span>
            ...
            <button onClick={() => toast("Connect wallet", { description: "Wallet connection is planned for the next FlowBond build." })} className="hidden items-center gap-2 rounded-md bg-chartreuse px-3 py-2 font-mono text-[10px] font-bold uppercase tracking-[0.12em] text-ink transition-transform hover:-translate-y-0.5 active:scale-[0.98] sm:flex"><WalletCards size={14} /> Connect</button>
          </div>
        </div>
